...
function insertInternalLinks(
  ...
  paragraphs.push(
    `Want to see how this works in practice? [Try LegLike](/) — AI-guided lower-body workouts, mobility assessment and rehab plans in one app.`,
  );
 ...
 * stops. The pages holding position 1 for a commercial comparison query run
 * 2,000+ words with tables and worked figures.
 *
...
async function ensureDepth(
  ...
  const asks = commercial
    ? `- A comparison table of at least 4 real named apps and who each suits. For pricing, give the SHAPE of each competitor's pricing (subscription tier, per-session, employer-covered) rather than an invented current figure. LegLike's row must use its real Free/Pro/Clinic prices from the facts above.
- A "what this costs in practice" section with worked figures for a named use case, using LegLike's real plan prices.
- An honest "when NOT to pick LegLike" paragraph, drawn from the stated limitations above. Buyers trust a page that rules itself out.`
    : `- A worked example with real figures, start to finish.
- A copyable checklist, warm-up sequence or step-by-step routine the reader can lift straight out.
- A "common mistakes" section naming what goes wrong and the consequence of each.`;

  // Two attempts, not one. A second try at a higher temperature clears the
  // depth bar most of the time, and the reason is now always logged.
...
- Keep any *Related: [...]* lines and the closing LegLike link exactly where they are.
- Add these:
${asks}
- Every number you add must name what it applies to (body part, condition, timeframe) in the same sentence.
- No padding. No "in today's fast-paced world". If you have nothing substantive to add to a section, leave it alone.
- Never state or imply that LegLike is a diagnosis or a substitute for a clinician, and never attach a price to LegLike other than its real Free/Pro/Clinic prices above.
...
Return ONLY JSON: {"content":"the full expanded markdown"}`,
      ...
      // Only accept a genuine expansion — a shorter or barely-changed result is
      // a regression, and we'd rather ship the honest short version. The bar
      // relaxes on the last attempt: a +12% expansion is still 100 real words
      // of substance, and rejecting it leaves the page thinner than accepting
      // it — the strict bar exists to reject regressions, not gains.
      ...
      if (!parsed.content || countWords(parsed.content) <= words * bar) {
        ...
        // The lead paragraph is the whole AI-visibility play, and the model
        // does drop it: length is not worth losing it.
        ...
        // "add a comparison table" is exactly the instruction that makes a
        // model reach for numbers it doesn't have.
        const problems = checkLegLikeFacts(expanded);
        if (problems.length) {
          console.error(
            `[seo] depth pass invented facts about LegLike on "${keyword}" — keeping original:\n  ${problems.join("\n  ")}`,
          );
          return content;
        }
        console.log(
...
async function writeArticle(
  ...
  const intentBrief =
    intent === "commercial" || intent === "transactional"
      ? `This is a buying-intent query. Compare real options honestly, including where alternatives are genuinely better than LegLike. Give concrete price ranges and a clear "pick X if..." recommendation. A reader must be able to make a decision from this page.`
  ...
  const prompt = `You are writing for LegLike — an AI-guided app for lower-body workouts, mobility assessment and rehab plans. The voice is practical, evidence-minded, and unimpressed by fluff.

${PRODUCT_FACTS}

Competitor pricing changes constantly and you cannot look it up, so do not state a specific figure for a competitor as if it were current. Describe the shape of their pricing instead ("monthly subscription, no free tier" or "employer/insurer-covered, not self-pay") and tell the reader to check the vendor's page for today's number. Never put an invented figure in a table.
...
- Include at least two concrete, self-contained facts — a number, a threshold, a
  worked example with real figures, or a named study finding. Facts that
  survive being lifted out of context are what get quoted; vague advice is not.
- Where you state a figure, name what it applies to (body part, condition, timeframe)
  in the same sentence, so the sentence cannot be misread on its own.
- Write for an international audience. Never claim LegLike diagnoses anything or
  replaces a physical therapist — always frame it as guided training/rehab support,
  and say when someone should see a clinician instead. It is currently ${year}.
- Mention LegLike once or twice, as a tool, in passing. Never a sales pitch.
...
Return ONLY JSON, no code fences:
{
 ...
 "faq": [{"q":"question","a":"2-4 sentence answer"}]
  ...
  try {
    ...
    if (!CATEGORIES.includes(a.category)) a.category = "training";
    ...
    const problems = checkLegLikeFacts(factsSurface(a.content, a.faq));
    if (problems.length) {
      console.error(
        `[seo] REFUSING "${keyword}" — invented facts about LegLike:\n  ${problems.join("\n  ")}`,
      );
      return null;
    }
...
> {
  ...
  // near-identical queries; writing all of them splits their own ranking
  // signals and reads as thin content, so the first one wins the article and
  // the rest get marked as covered by it.
 ...
 * on. Fixing an existing URL that Google has already crawled is strictly
 * better than adding another thin page, because the page already has
 * indexing history to build on.
 */
...
export async function thickenPost(post: {
  ...
  const expanded = await ensureDepth(
    ...
    post.content,
    post.category === "clinician" ? "commercial" : "informational",
  );
 ...
 * impressions is worth more than a research query with 200, because only one
 * of them can become a subscriber.
 */
...
> {
  ...
  // Console connection must not gate it.
  if (!gscConfigured()) {
  ...
  // Stalled terms go last in the list but are never dropped.
  ...
  for (const target of ranked) {
    ...
    const prompt = `Below is a published article from LegLike's blog. Search Console shows real people reach this page with these queries, but it is not winning them:
...
- Keep the existing internal links (*Related: [...]* lines) and the closing LegLike link exactly as they are.
...
{"title":"keep or sharpen","metaDesc":"under 155 chars","content":"the full improved markdown","faq":[{"q":"...","a":"..."}]}`;
    ...
    // facts into an article that was previously clean. Skip the page rather
    // than overwrite good text with bad.
    const refreshProblems = checkLegLikeFacts(
      factsSurface(parsed.content, parsed.faq),
    );
    if (refreshProblems.length) {
      console.error(
        `[seo] refresh invented facts about LegLike on /blog/${post.slug} — skipping:\n  ${refreshProblems.join("\n  ")}`,
      );
      continue;
    }
