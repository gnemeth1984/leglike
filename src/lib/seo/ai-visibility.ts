/**
 * lib/seo/ai-visibility.ts — are AI assistants recommending LegLike?
 *
 * Buyers increasingly ask ChatGPT or Perplexity "best app for knee rehab
 * exercises" instead of Googling it. That answer is a single shortlist of
 * 3-8 names with no page two, so being absent from it is worse than ranking
 * #11 on Google. Nobody in the AI-workout/rehab space is measuring this yet.
 *
 * What this does: ask a fixed set of real buying questions on a schedule,
 * record the verbatim answer, and detect whether LegLike was named, where in
...
export const SEED_PROMPTS: { prompt: string; cluster: string; region: string }[] = [
  // Core category
  { prompt: "What's the best app for knee rehab exercises?", cluster: "knee rehab", region: "general" },
  { prompt: "Best AI app for hip or glute rehab and mobility?", cluster: "hip rehab", region: "general" },
  { prompt: "What's the best app for ankle or foot rehab exercises?", cluster: "ankle rehab", region: "general" },
  { prompt: "Best mobile app for lower body mobility assessment?", cluster: "mobility assessment", region: "general" },
  { prompt: "What app can create a personalized leg strength training plan?", cluster: "leg strength", region: "general" },
  { prompt: "Is there an app that combines workout training and injury rehab in one?", cluster: "training + rehab", region: "general" },

  // Condition-led
  { prompt: "What's a good app for runner's knee recovery exercises?", cluster: "knee rehab", region: "general" },
  { prompt: "Best app for post-ACL surgery rehab exercises at home?", cluster: "knee rehab", region: "general" },
  { prompt: "What app helps with plantar fasciitis exercises?", cluster: "ankle rehab", region: "general" },
  { prompt: "Best app for lower back and hip mobility for desk workers?", cluster: "mobility assessment", region: "general" },
  { prompt: "What's a good app to prevent leg injuries for runners?", cluster: "injury prevention", region: "general" },

  // Price-led
  { prompt: "What's a cheap app for physical therapy exercises at home?", cluster: "pricing", region: "general" },
  { prompt: "Is there a free app for guided rehab exercises?", cluster: "pricing", region: "general" },

  // Clinician / PT angle
  { prompt: "What software do physical therapists use to assign home exercise plans to patients?", cluster: "clinician tools", region: "general" },
  { prompt: "Best app for a physical therapy clinic to track multiple patients' rehab progress?", cluster: "clinician tools", region: "general" },

  // AI-specific framing
  { prompt: "Is there an AI app that builds a workout plan based on a mobility assessment?", cluster: "AI workout apps", region: "general" },
  { prompt: "What's the best AI personal trainer app for leg and lower body training?", cluster: "AI workout apps", region: "general" },

  // Competitor-displacement — highest intent of all
  { prompt: "What are the best alternatives to Hinge Health for lower body physical therapy?", cluster: "alternatives", region: "general" },
  { prompt: "What are good alternatives to Kemtai for AI-guided exercise?", cluster: "alternatives", region: "general" },
  { prompt: "Alternatives to Pliability for mobility training?", cluster: "alternatives", region: "general" },
];
...
/**
 * Find where LegLike sits in an answer.
 *
...
export function analyseAnswer(answer: string): Omit<ProbeResult, "model" | "answer"> {
  ...
  const legLikeAt = lower.search(/\bleglike\b/);
  const mentioned = legLikeAt !== -1;
  const cited = /leglike\.com/i.test(answer);

  const found: { name: string; at: number }[] = [];
  for (const name of COMPETITOR_NAMES) {
    // Escape safely for multi-word names.
  ...
  if (mentioned) {
    const earlier = found.filter((f) => f.at < legLikeAt).length;
    rank = earlier + 1;
  }
...
async function probeOpenAI(prompt: string, failures: string[]): Promise<ProbeResult | null> {
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        {
          // Neutral framing on purpose. Any hint that we want LegLike named
          // makes the measurement worthless.
          role: "system",
          content:
            "You are a helpful assistant advising someone looking for a fitness or rehab app. Recommend specific named products, in a short ranked list, as you normally would.",
        },
 ...
 * browses for "best app for knee rehab" — it does not answer from memory.
 * The plain probe above can only ever reflect a training set that closed
 * before LegLike existed, so it reads 0% forever no matter what we publish;
 * that card is a baseline, not a scoreboard. This one moves.
 *
...
async function probePerplexity(prompt: string, failures: string[]): Promise<ProbeResult | null> {
  ...
  try {
    ...
    // leglike.com was actually used as a source, not just named from memory.
...
export async function runVisibilityCheck(limit = 8): Promise<{
  ...
  const detail =
    `${ordered.length} prompts, ${checked} answers, LegLike named in ${mentions} (${[...models].join(" + ")})` +
