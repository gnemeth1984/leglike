...
function DeltaPill({
  ...
  const tone = flat
    ? "text-neutral-400 bg-neutral-800/60 border-neutral-700"
    : good
...
export function SeoTrendChart({ trend, deltas, connected }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    return slice.map((r, i) => ({
      ...
      label: new Date(`${r.date}T00:00:00Z`).toLocaleDateString("en-US", {
        day: "numeric",
        month: "short",
      }),
    }));
  ...
  if (!connected) {
    return (
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
        <h3 className="text-sm font-semibold text-white">Search performance</h3>
        ...
        <div className="h-64 w-full sm:h-80">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={rows} margin={{ top: 4, right: 4, left: -18, bottom: 0 }}>
              ...
              />
              <Legend wrapperStyle={{ fontSize: 11, color: "#a3a3a3" }} iconType="circle" />
              <Area
                yAxisId="impr"
                type="monotone"
                dataKey="impressions"
