...
const MODEL_NOTE: Record<string, string> = {
  "gpt-4o-mini":
    "Answers from a frozen training set that closed before LegLike existed, so this stays at 0% no matter what you publish. It is the baseline, not the scoreboard.",
  "gpt-4o-mini-search":
...
export function AiVisibilityPanel() {
  ...
  async function runCheck() {
    ...
    try {
      ...
      setMessage(
        json.ok
          ? `Asked ${json.checked} questions — LegLike named in ${json.mentions}.`
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-neutral-800 bg-neutral-900/60">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-neutral-800 p-4">
          <div className="min-w-0">
            <h3 className="flex items-center gap-2 text-sm font-semibold text-white">
              <Bot className="h-4 w-4 text-lime-400" />
              AI visibility — do assistants recommend LegLike?
            </h3>
        ...
        {message && (
          <div className="border-b border-neutral-800 bg-neutral-800/40 px-4 py-2 text-sm text-neutral-300">
            {message}
          </div>
        )}
        ...
        {!data.perplexity && (
          <div className="flex items-start gap-2 border-b border-amber-500/20 bg-amber-500/10 px-4 py-2.5">
            ...
            <p className="text-[11px] leading-relaxed text-neutral-400">
              ...
              <code className="rounded bg-neutral-800 px-1">PERPLEXITY_API_KEY</code> to also check a
        ...
        {data.lastRun && !data.lastRun.ok && (
          <div className="flex items-start gap-2 border-b border-rose-500/20 bg-rose-500/10 px-4 py-2.5">
            <XCircle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-rose-500" />
            <p className="text-[11px] leading-relaxed text-neutral-400">
              Last check hit an error, so a model may be missing below:{" "}
              <span className="font-medium text-neutral-200">{data.lastRun.detail}</span>
            </p>
          </div>
        )}
        ...
        {data.checks === 0 ? (
          ...
          <>
            <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3">
              {data.byModel.map((m) => (
                <div key={m.model} className="rounded-xl border border-neutral-800 p-3">
                  <p className="text-[11px] uppercase tracking-wide text-neutral-500">
                    {MODEL_LABEL[m.model] ?? m.model}
                  </p>
                  <div className="mt-1 flex items-baseline gap-2">
                    <p className="text-2xl font-bold text-white">{m.sharePct.toFixed(0)}%</p>
                  ...
                  <p className="mt-2 text-[11px] text-neutral-500">
                    ...
                    {m.cited ? ` · cited leglike.com ${m.cited}×` : ""}
                  </p>
                  {MODEL_NOTE[m.model] && (
              ...
              <div className="border-t border-neutral-800 p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {data.competitors.map((c) => (
                    <span
                      key={c.name}
                      title={`Named in ${c.total} answers · ${c.beatUs} where LegLike was absent`}
                      className="rounded-md border border-neutral-800 bg-neutral-800/40 px-2 py-1 text-[11px] text-neutral-300"
                    >
                      {c.name} <span className="font-semibold text-white">{c.sharePct.toFixed(0)}%</span>
                    </span>
                  ))}
                </div>
                <p className="mt-2 text-[11px] text-neutral-500">
            ...
            <div className="border-t border-neutral-800">
              <h4 className="px-4 pb-1 pt-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">
                Latest answers
              </h4>
                  ...
                  <div key={r.id} className="px-4 py-2.5">
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block text-sm text-neutral-200">{r.prompt}</span>
                        <span className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-neutral-500">
                          ...
                          {r.mentioned && r.rank && (
                            <span className="font-medium text-lime-400">#{r.rank} in list</span>
                          )}
                          {r.cited && (
                      ...
                      <ChevronDown
                        className={`mt-0.5 h-4 w-4 shrink-0 text-neutral-600 transition ${
                          open === r.id ? "rotate-180" : ""
                        }`}
                      />
                    </button>
                    {open === r.id && (
                      <pre className="mt-2 max-h-72 overflow-auto whitespace-pre-wrap rounded-lg bg-neutral-950 p-3 text-[11px] leading-relaxed text-neutral-300">
                        {r.answer}
                      </pre>
                    )}
                  </div>
            ...
            {data.lastRun && (
              <p className="border-t border-neutral-800 px-4 py-2.5 text-[11px] text-neutral-600">
                Last check {new Date(data.lastRun.at).toLocaleString("en-US")} —{" "}
                {data.lastRun.detail}
              </p>
            )}
          </>
